...
// Economic categories first, then others
const categories: EventCategory[] = [
  'tariff', 'trade', 'economic',
  'diplomatic', 'statement',
  'conflict', 'military',
  'terrorism', 'disaster', 'earthquake', 'prediction',
]
...
const categoryAccent: Partial<Record<EventCategory, string>> = {
  tariff: 'var(--accent-amber)',
    ...
    <div className="flex flex-col gap-2 pb-2 border-b border-[var(--border)]">
      ...
      <div className="relative">
        ...
        <input
          ...
          className="w-full pl-8 pr-3 py-1.5 rounded-md bg-[var(--bg-tertiary)] border border-[var(--border)] text-xs text-[var(--text-primary)] placeholder:text-[var(--text-secondary)] focus:outline-none focus:border-[var(--accent-amber)]"
        />
      </div>
        ...
        {categories.map((c) => {
          ...
          return (
            <button
              ...
              style={active ? { background: accent ?? 'var(--accent-amber)' } : undefined}
            >
              {dict.categories[c] ?? c}
            </button>
          )
